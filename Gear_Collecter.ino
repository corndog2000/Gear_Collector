...
void loop()
{
  ...
  if (joystickControl)
  {
    ...
    else if (motorsRunning <= 0 && XboxWirelessControl) //Xbox Controller
    {
      ...
      if (Xbox.XboxReceiverConnected) {
        if (Xbox.Xbox360Connected[controlNum]) {
          ...
          collect = (Xbox.getButtonPress(A, controlNum));
          unCollect = (Xbox.getButtonPress(Y, controlNum));
          lift = (Xbox.getButtonPress(X, controlNum));
          unLift = (Xbox.getButtonPress(B, controlNum));
    ...
    backLeftSpeed = frontLeftSpeed = backRightSpeed = frontRightSpeed = joyY;   //Sets the speed for all motors based on the Forward/Reverse of the joystick
      if (collect) {     //Strafe Left
        frontLeftSpeed = 2000;
      }
      else if (unCollect) {      //Strafe Right
        frontLeftSpeed = 1000;
      }
      else if (lift) {     //Zero point turn Left
        frontRightSpeed = 1600;  //lowNeutral for backwords movement
      }
      else if (unLift) {      //Zero point turn Right
        frontRightSpeed = 1300;
      }
  }
